    ...
    var NFT_Service_Add = function(){
        ...
        var handleFormRepeater  = function(){
            $('.day-delete').on('click', function(e){
                ...
                var actionMode      = inputContainer.find('input[name="'+inputName+'[action]')

                inputContainer.slideUp();
                actionMode.val('delete');
            })

            $('.day-deactivate').on('click', function(e){
                var inputName       = $(this).attr('data-input-name');
                var inputContainer  = $(this).closest('.day-hours-container');
                var actionMode      = inputContainer.find('input[name="'+inputName+'[action]"]');

                if( actionMode.val() == 'activate'){
                    actionMode.val('deactivate');
                    $(this).find('.deactivate').addClass('d-none');
                    $(this).find('.activate').removeClass('d-none');
                }
                else if( actionMode.val() == 'deactivate' ){
                    actionMode.val('activate');
                    $(this).find('.activate').addClass('d-none');
                    $(this).find('.deactivate').removeClass('d-none');
                }
            })
