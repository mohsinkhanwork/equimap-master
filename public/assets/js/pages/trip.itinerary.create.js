...
jQuery(function($){
    var NFT_Trip_Add_Itinerary = function(){
        var form;
        var submitButton;
        var formUrl;
        var redirectUrl;
                ...
                axios.post( formUrl, new FormData( form[0] ) ).then( function( response ){
                    if( response && response.data.status == 200 ){
                        ...
                        $.each( response.data.content, function( key, value ){
                            var key     = key.replaceAll('.', '-').replaceAll('/','-');
        ...
        return {
            init: function () {
                form        = $("#em_create_itinerary");
                formUrl     = form.attr('action');
                redirectUrl = form.attr('data-redirect-url');
                submitButton= $("#submit_em_create_itinerary");
