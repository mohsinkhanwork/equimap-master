...
class Theme
{
    ...
    function __construct(){
        ...
        if( isset( $locales[$current] ) && isset( $locales[$current]['dir'] ) ){
            $this->setDirection( $locales[$current]['dir'] );
        }
    }
    ...
    {
        $configs = config('settings.KT_THEME_ASSETS.global.'.$type);
        foreach( $configs as $key => $value ){
            $configs[$key] = $this->extendCssFilename( $value );
        }
