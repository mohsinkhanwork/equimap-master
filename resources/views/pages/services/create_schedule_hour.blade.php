    ...
    <div class="row g-5 g-xl-10 mb-5 mb-xl-10">
        ...
        <div class="card card-flush h-md-50 mb-5 mb-xl-10">
            ...
            <div class="card-body d-flex flex-column justify-content-end p-5">
                <!--begin::Form-->
                <form autocomplete="off" data-redirect-url="{{ route('acp.services.index') }}" id="em_create_schedule" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('acp.services.store_schedule_hour', ['service_id' => $service->id ] ) }}" method="post">
                    @foreach( $days as $day => $title )
                        ...
                        <div class="row day-container">
                            <div class="d-flex justify-content-between">
                                <div class="day-active-container fw-bolder fs-4 text-primary m-2">
                                    <label for="{{ $inputName }}[active]" class="form-check-label">{{ $title }}</label>
                                </div>
                                <!--begin::Form group-->
                                <div class="mb-7">
                                    <a data-repeater-create href="javascript:;" class="day-hours-repeat btn btn-flex btn-light-primary">
                                    ...
                                    <div class="day-hours-container">
                                        <div class="d-flex justify-content-start gap-3 flex-md-row flex-column">
                                            ...
                                            <div class="d-flex flex-row justify-content-between gap-3 mb-7">
                                                ...
                                                <a href="javascript:;" data-input-name="{{ $savedInputName }}" class="day-deactivate btn btn-light-dark p-4">
                                                    <span class="deactivate {{ $schedule->active != 1 ? ' d-none' : ''}}">
                                                        <i class="fas fa-solid fa-link-slash fs-4<"></i>
                                                        {{ __('acp/general.layout.deactivate') }}
                                                    </span>
                                                    <span class="activate {{ $schedule->active == 1 ? ' d-none' : ''}}">
                                                        <i class="fas fa-solid fa-link fs-4"></i>
                                                        {{ __('acp/general.layout.activate') }}
                                                    </span>
                                                </a>
                                            ...
                                            <input type="hidden" class="day-active-toggle" name="{{ $savedInputName }}[action]" value="{{ $schedule->active == 1 ? 'activate' : 'deactivate'}}" />
