<div class="dropdown">
    ...
    <div aria-labelledby="menu-{{ $id }}" class="dropdown-menu menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
        ...
        <div class="menu-item px-3">
            @can('edit package')
                <a href="{{ route('acp.packages.edit', ['package_id' => $id ] ) }}" class="btn btn-white btn-sm">
                    Edit
                </a>
            @endcan
            @can('delete package')
                <form method="POST" class="em_delete_package" name="em_delete_package" action="{{ route('acp.packages.destroy', [ 'package_id' => $id ] ) }}">
                    @csrf
                    @method('DELETE')
